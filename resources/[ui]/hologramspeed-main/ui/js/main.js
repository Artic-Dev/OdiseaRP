...
const fuelCompost = document.getElementById("fuelCompost");
const fuelCompostBarr = document.getElementById("fuelCompostBarr");
...
window.addEventListener("message", function (ev) {
	...
	if (data.combustible) {
		if (data.combustible < 30) {
			if (!reserva) {
				fuelCompost.classList.add('reserva')
				reserva = true
			...
			if (reserva) {
				fuelCompost.classList.remove('reserva')
				
				reserva = false
			}
		}

		fuelCompostBarr.style.height = `${data.combustible}%`
	}
