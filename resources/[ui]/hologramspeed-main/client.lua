...
function AdaptarHUD() 
	local vehicle = GetVehicleClass(GetVehiclePedIsIn(PlayerPedId(), true))
	local coche = vec3(2.5, -1, 0.85)
	local moto = vec3(1.5, -1, 0.85)
	local comercial = vec3(2.9, -5, 2)
	if vehicle == 0  then 
		return coche
	...
	elseif vehicle == 9 then 
		return coche
	elseif vehicle == 10 then 
		return coche
	elseif vehicle == 11 then 
		return coche
	elseif vehicle == 12 then 
		return coche
	elseif vehicle == 13 then 
		return moto
	elseif vehicle == 14 then 
		return coche
	elseif vehicle == 15 then 
		return coche
	elseif vehicle == 16 then 
		return coche
	elseif vehicle == 17 then 
		return coche
	elseif vehicle == 18 then 
		return coche
	elseif vehicle == 19 then 
		return coche
	elseif vehicle == 20 then 
		return comercial
	elseif vehicle == 21 then 
		return coche
	end
...
RegisterCommand("mycar", function(_, args)	
	SendChatMessage(GetVehicleClass(GetVehiclePedIsIn(PlayerPedId(), true)))
end, false)
				...
				if DoesEntityExist(currentVehicle) and GetPedInVehicleSeat(currentVehicle, -1) == playerPed then
					...
					repeat
						...
						EnsureDuiMessage {
							...
							combustible = tostring(math.ceil(GetVehicleFuelLevel(GetVehiclePedIsIn(PlayerPedId(), true)))),
